...
public final class Acquiror implements AutoCloseable {
	 ...
	 * Treating it as immediately fatal (the pre-2026-08-06 behaviour) meant a
	 * single transient hiccup during a periodic root-sync or delta broadcast
	 * turned into a permanent, silent delivery failure: nothing ever retried
	 * that specific hash again, and since a node's lattice root routes through
	 * the same unresolved branch on every later update too, EVERY subsequent
	 ...
	 * always-retried {@code pullPath} full-sync path). Found live 2026-08-06
	 * diagnosing "dev"/"meta" dbase nodes drifting apart indefinitely.
	 */
