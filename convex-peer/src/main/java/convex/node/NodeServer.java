...
public class NodeServer<V extends ACell> implements Closeable {
	...
	private CompletableFuture<Message> acquireLatticeMessage(Message message,
		...
		try {
			...
			// first access and can itself throw MissingDataException. Found live
			// 2026-08-06: that exact throw, if it happens on THIS guard's own
			// payload.get(2) call, escaped to the generic catch below (which just
			// gives up -- no acquisition attempted at all) instead of the
			// MissingDataException-aware recovery path a few lines down, turning
			// every periodic root sync referencing not-yet-local data into an
			// immediate, permanent, silently-never-retried delivery failure --
			// repeating identically every ROOT_SYNC_INTERVAL forever, since
			// nothing about the node's state ever changes to make the SAME lazy
			// access succeed differently next time.
			AVector<?> payload = RT.ensureVector(message.getPayload());
			...
			try {
				...
				// the envelope-decode catch above -- NOT payload.get(3).getHash().
				// completeLatticeMessage's own payload.get(3) access is what threw
				...
				// payload.get(3) again here hits the exact same still-unresolved
				// ref and throws again, uncaught, escaping to the generic catch
				// below with no acquisition ever attempted. Found live 2026-08-06
				// as a second occurrence of the same underlying mistake fixed
				// above the same day: re-deriving a hash via another risky lazy
				// access instead of using the one the exception already handed
				// us. The recursive acquireLatticeMessage call below iterates
				// again after this fetch, so acquiring just this one missing
				// piece per round (rather than needing the true tree root in one
				// shot) is sufficient -- it converges the same way the
				// envelope-decode path above already does.
				return acquireHash(connection, acquisitionStore, e.getMissingHash())
	 ...
	 * even if the merge was rejected, absent, or a no-op — any local writes
	 * not yet checkpointed must not be skipped just because this particular
	 * pull didn't change anything.
	 *
	 * <p>The caller must not mutate {@code path} while the returned operation
	 * is outstanding.
	 *
	 * @param convex Convex connection to the peer node
